...
### Sample Size Justification when Minimizing/Balancing Error Rates
...
Let us illustrate this using the example of researchers studying an effect of *d* = 0.5 with a two sample *t*-test. We can estimate the sample size as well as the alpha and power they would require to achieve a weighted combined error rate of 5% when weighting Type 1 errors and Type 2 errors equally strong. Figure \ref{fig:error-plot} shows alpha, beta, and the weighted combined error rate as a function of sample size when the alpha level is minimized. We can see that a weighted combined error rate of 5% (red line) is achieved at a sample size of `r res7$samplesize`. By iteratively optimizing the alpha level for different sample sizes and stopping when the desired error rate is reached, we can justify our sample size and the alpha level at the same time. 
